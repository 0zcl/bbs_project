...
def get_new_msgs(request):  # 用户接收消息
    ...
    msg_size = q_obj.qsize()  # 队列长度
    msg_list = []
    if msg_size > 0:  # 有消息
        for msg in range(msg_size):
            msg_list.append(q_obj.get())  # 从队列取消息(先入先出)
